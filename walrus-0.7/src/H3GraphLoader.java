...
public class H3GraphLoader {
    ...
    // Returns List<String>.
    public List loadColoringTreeQualifiers(Graph graph) {

        List retval = new ArrayList();

        QualifierIterator iterator =
                graph.getQualifiersByType(COLORING_TREE_QUALIFIER);
        while (!iterator.atEnd()) {
            QualifierAttributeIterator aIterator = iterator.getAttributes();

            while (!aIterator.atEnd()) {
                retval.add(aIterator.getName());
                aIterator.advance();
            }
    ...
    public List loadNumericTreeQualifiers(Graph graph) {

        List retval = new ArrayList();

        QualifierIterator iterator =
                graph.getQualifiersByType(INTEGER_TREE_QUALIFIER);
        while (!iterator.atEnd()) {
            QualifierAttributeIterator aIterator = iterator.getAttributes();
    ...
    // PRIVATE METHODS
    ////////////////////////////////////////////////////////////////////////
    private void findSpanningTreeQualifierAttributes(Graph graph, String spanningTree)
            throws InvalidGraphDataException {
        ...
        while (!qualifierAttributeIterator.atEnd()) {
            ...
            if (name.equals(ROOT_ATTRIBUTE)) {
                ...
                checkAttributeType(graph, SPANNING_TREE_QUALIFIER, ROOT_ATTRIBUTE,
                        m_rootAttribute, ValueType.BOOLEAN);
            } else if (name.equals(TREE_LINK_ATTRIBUTE)) {
                foundTreeLinkAttribute = true;
                m_treeLinkAttribute =
                        qualifierAttributeIterator.getAttributeID();
                checkAttributeType(graph, SPANNING_TREE_QUALIFIER, TREE_LINK_ATTRIBUTE,
    ...
    ///////////////////////////////////////////////////////////////////////
    private void checkAttributeType(Graph graph, String qualifierType, String attributeName,
    ...
    private BitSet createTreeLinksMap(Graph graph, int treeLinkAttribute) {
        ...
        AttributesByAttributeIterator iterator =
                graph.getAttributeDefinition(treeLinkAttribute).getLinkAttributes();
        while (!iterator.atEnd()) {
    ...
    private static final String COLORING_TREE_QUALIFIER = "coloring_tree";
    private static final String INTEGER_TREE_QUALIFIER = "integer_tree";

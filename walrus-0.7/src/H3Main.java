...
public class H3Main {
    ...
    public H3Main() {
        ...
        loadGraphSetProperties();

        try {

            m_graphSetProperties = new H3GraphSetProperties((graphSetProperties));
        } catch (Exception e) {

            JOptionPane dialog = new JOptionPane();
            dialog.showMessageDialog(m_frame, e.getMessage(), "Error",
    ...
    public H3Main(String graphSetGraphName, int[] nodeArray) {
        initializeCanvas3D();
        ...
        m_frame.show();
        try {
            if (m_graphSetProperties != null) {
                if (m_graphSetProperties.getGraphPath(graphSetGraphName).compareTo("") != 0) {
                    handleOpenDiskFileRequest(m_graphSetProperties.getGraphPath(graphSetGraphName));
                    setM_loadedGraph(graphSetGraphName);
                    handleStartRenderingRequest();
                    m_eventHandler.m_drillDownEventHandler.drillDownGraphToChildren(nodeArray, false);
                } else {
                    JOptionPane dialog = new JOptionPane();
                    dialog.showMessageDialog(m_frame, "Could not load the graph " + graphSetGraphName, "Load Failure",
    ...
    private RenderingConfiguration createRenderingConfigurationSnapshot() {
        ...
        retval.nodeAttributeColoring = m_nodeAttributeColoringMenuItem.isSelected();
        ...
        retval.nodeColors = m_colorSchemeMenu.createNodeColorsConfigurationSnapshot(m_graphLoader, m_backingGraph);
    ...
    private String createNodeFullIdentifier(String[] values) {
        StringBuffer buffer = new StringBuffer();


        for (int i = 0; i
                < m_nodeLabelAttributes.size(); i++) {
            if (i > 0) {
                buffer.append("; ");
    ...
    private void initNodeFullIdentifiers() {

        ListIterator iterator = m_nodeLabelAttributes.listIterator();

        int[] nodeAttributeIds = new int[m_nodeLabelAttributes.size()];
        String[] nodeAttributeNames = new String[m_nodeLabelAttributes.size()];
        ...
        while (iterator.hasNext()) {
            ...
            AttributeDefinitionIterator attIterator = m_backingGraph.getAttributeDefinition(name);
            if (!attIterator.atEnd()) {
                nodeAttributeIds[index] = attIterator.getID();
                nodeAttributeNames[index] = name;
                index++;
            }
        }
        NodeLabelConstructor nodeLabelInterface = new NodeLabelConstructor(m_backingGraph, nodeAttributeIds);

        int noNodes = m_graph.getNumNodes();

        for (int i = 0; i < noNodes; i++) {

            int node = m_graph.getNodeID(i);
            String[] attributeValues = nodeLabelInterface.extractValues(node, false);
            String nodeFullIdentifier = createNodeFullIdentifier(attributeValues);
            m_graph.setNodeFullIdentifier(node, nodeFullIdentifier);
    ...
    private void populateMenus(Graph graph) {
        ...
        /*m_nodeColorAttributes = m_graphLoader.loadColoringTreeQualifiers(graph);
        {
        ListIterator iterator = m_nodeColorAttributes.listIterator();
        while (iterator.hasNext()) {
        String name = (String) iterator.next();
        JMenu attributeMenu = new JMenu(name);
        
        String[] nodeAttributes = getDistinctAttributeValues(graph, name);
        if (nodeAttributes != null) {
        for (int i = 0; i < nodeAttributes.length; i++) {
        JMenu attributeValueMenu = new JMenu(nodeAttributes[i]);
        attributeMenu.add(attributeValueMenu);
        Map nodeAttributeMenuMap = new HashMap();
        m_nodeAttributeColorSelection = new ColorSelection(attributeValueMenu, nodeAttributeMenuMap, m_colorSchemeMenu.m_fixedColors);
        
        }
        }
        m_nodeAttributeColorMenu.add(attributeMenu);
    ...
    /* public String[] getDistinctAttributeValues(Graph graph, String attributeName) {
    AttributeDefinitionIterator adi = graph.getAttributeDefinition(attributeName);
    NodeLabelConstructor nodeAttributeInterface = new NodeLabelConstructor(graph, new int[]{adi.getID()});
    
    HashSet<String> distinctAttributeValues = new HashSet<String>();
    for (int i = 0; i < graph.getNumNodes(); i++) {
    int node = graph.getNode(i).getObjectID();
    
    if (nodeAttributeInterface.extractValues(node, false).length > 0) {
    String attributeValue = nodeAttributeInterface.extractValues(node, false)[0];
    distinctAttributeValues.add(attributeValue);
    }
    }
    
    
    return distinctAttributeValues.toArray(new String[]{});
    }*/
    ///////////////////////////////////////////////////////////////////////
    ...
    private void colorNodes(ColorConfiguration configuration) {
        ...
        setNodeTransparencyEnabled(configuration.isTransparent
                && configuration.scheme != ColorConfiguration.RGB && configuration.scheme != ColorConfiguration.NUMERIC);
        ...
        switch (configuration.scheme) {
            ...
            case ColorConfiguration.RGB:
                ...
                break;
            case ColorConfiguration.NUMERIC:
                colorNodesOnAttributeRange(configuration.numericAttribute);
                break;
    ...
    //   that colorNumeric nodes and links.  It turns out that exceptions are not
    ...
    private void colorNodesRGB(String colorAttribute) {
        ...
        for (int i = 0; i < numNodes; i++) {
            ...
            try {
                ...
                // Nothing to do--simply use the default colorNumeric.
            }
            m_graph.setNodeColor(i, color);
        }
    }

    private void colorNodesOnAttributes(HashMap<Integer, ColorConfiguration> nodeColors) {
        ...
        for (int i = 0; i < numNodes; i++) {
            ...
            if (nodeColors.get(nodeID) != null) {
                color = nodeColors.get(nodeID).fixedColor;
            }
            m_graph.setNodeColor(i, color);
        }
    }

    private void colorNodesOnAttributeRange(String colorAttribute) {
        ...
        int[] nodeColorNumericValues = new int[numNodes];
        for (int i = 0; i < numNodes; i++) {
            int colorNumeric = defaultColor;
            try {
                int nodeID = m_graph.getNodeID(i);
                ValueIterator iterator =
                        m_backingGraph.getNodeAttribute(nodeID, attribute);
                colorNumeric = extractColorAttributeValue(iterator);
                nodeColorNumericValues[i] = colorNumeric;
                if (colorNumeric < minimum) {
                    minimum = colorNumeric;
                }
                if (colorNumeric > maximum) {
                    maximum = colorNumeric;
                }
        ...
        if (minimum < maximum) {
            ...
            int[] nodeRGBValues = GetRGBValuesFromNumeric(minimum, maximum, nodeColorNumericValues);

            for (int i = 0; i < nodeRGBValues.length; i++) {
                m_graph.setNodeColor(i, nodeRGBValues[i]);
            }
        }
    }

    private int[] GetRGBValuesFromNumeric(int MinValue, int MaxValue, int[] nodeColorValues) {

        int[] rgbValues = new int[nodeColorValues.length];
        double MinVisibleWaveLength = 350.0;
        double MaxVisibleWaveLength = 650.0;

        for (int i = 0; i < nodeColorValues.length; i++) {
            double Wavelength = ((double) (nodeColorValues[i] - MinValue) / (double) (MaxValue - MinValue))
                    * ((MaxVisibleWaveLength - MinVisibleWaveLength))
                    + MinVisibleWaveLength;

            rgbValues[i] = WavelengthToRGB(Wavelength);
    ...
    private double Adjust(double Color, double factor, double IntensityMax, double Gamma) {
        ...
        if (Color == 0.0) {
            ...
            RESULT = Math.round(IntensityMax * Math.pow(Color * factor, Gamma));
        }
        return RESULT;
    }
    ...
    private int WavelengthToRGB(double Wavelength) {
        ...
        if (Wavelength > 379 & Wavelength < 440) {
            ...
            Green = 0.0;
            Blue = 1.0;
        } else if (Wavelength > 439 & Wavelength < 490) {
            Red = 0.0;
            Green = (Wavelength
                    - 440) / (490 - 440);
            Blue = 1.0;
        } else if (Wavelength > 489 & Wavelength < 510) {
            Red = 0.0;
            Green = 1.0;
            Blue = -(Wavelength
                    - 510) / (510 - 490);
        } else if (Wavelength > 509 & Wavelength < 580) {
            Red = (Wavelength
                    - 510) / (580 - 510);
            Green = 1.0;
            Blue = 0.0;
        } else if (Wavelength > 579 & Wavelength < 645) {

            Red = 1.0;
            Green = -(Wavelength
                    - 645) / (645 - 580);
            Blue = 0.0;
        } else if (Wavelength > 644 & Wavelength < 781) {
            Red = 1.0;
            Green = 0.0;
            Blue = 0.0;
        } else {
        ...
        if (Wavelength > 379 & Wavelength < 420) {
            factor = 0.3 + 0.7 * (Wavelength - 380) / (420 - 380);
        } else if (Wavelength > 419 & Wavelength < 701) {
            factor = 1.0;
        } else if (Wavelength > 700 & Wavelength < 781) {
            factor = 0.3 + 0.7 * (780 - Wavelength);
        } else {
            factor = 0.0;
        }
        ...
        return NormalizeAndMakeColor(R, G, B);
    ...
    private int NormalizeAndMakeColor(double R, double G, double B) {
    ...
    private int extractColorAttributeValue(ValueIterator iterator) {
        int retval = 0;
        ...
        switch (iterator.getType().getType()) {
            ...
            case ValueType._DOUBLE:
                retval = (int) (iterator.getDoubleValue() * ROUND_OFF_MASK);
                break;
    ...
    private void handleResetRenderingRequest() {
        ...
        m_graph.initNodeVisibleDepth();
        m_graph.computeVisibleDepth();
        initNodeFullIdentifiers();
    ...
    private void handleEnableSubtreeToParentRequest(int node) {
        m_graph.enableVisibilityToParent(node);

        updateDisplayNarrowingMenusAndRefresh();
    }

    private void handleEnableSubtreeToSpecificChildrenRequest(int node, int[] children) {
        m_graph.enableVisibilityToSpecificChildren(node, children);
    ...
    ///////////////////////////////////////////////////////////////////////
    private void deActivateLabelsOnSubTree(H3LabelNode node) {

        int index = m_labelledNodes.indexOf(node);
        if (index != -1) {
            node.setIsLabelled(false);
            m_labelledNodes.set(index, node);


            int start = m_graph.getNodeChildIndex(node.getNode());
            int nontreeIndex = m_graph.getNodeNontreeIndex(node.getNode());
            for (int i = start; i < nontreeIndex; i++) {
                int child = m_graph.getLinkDestination(i);

                if (m_graph.checkNodeVisible(child)) {
                    H3LabelNode childWrapNode = new H3LabelNode(child, false);
                    int actualIndex = m_labelledNodes.indexOf(childWrapNode);
                    H3LabelNode actualChildNode = null;

                    if (actualIndex != -1) {
                        actualChildNode = m_labelledNodes.get(actualIndex);
                    }

                    if (actualChildNode != null) {
                        deActivateLabelsOnSubTree(actualChildNode);
                    }
                }
    ...
    private void deActivateLabel(H3LabelNode node) {
    ...
    private void activateLabel(H3LabelNode node, int nodeReferenceLevel) {

        if (m_graph.checkNodeVisible(node.getNode())) {
            boolean success = m_eventHandler.setLabel(node);
            node.setIsLabelled(success);
            int index = m_labelledNodes.indexOf(node);
            if (index == -1) {
                m_labelledNodes.add(node);
            } else {
                m_labelledNodes.set(index, node);
            }

            boolean areImmediateChildrenLabeled = node.getAreImmediateChildrenLabeled();
            if (nodeReferenceLevel != 0 && areImmediateChildrenLabeled) {
                nodeReferenceLevel--;
                ...
                for (int i = start; i < nontreeIndex; i++) {
                    int child = m_graph.getLinkDestination(i);
                    if (nodeReferenceLevel != 0) {
                        activateLabel(new H3LabelNode(child, true), nodeReferenceLevel);
                    } else {
                        activateLabel(new H3LabelNode(child, false), nodeReferenceLevel);
                    }
                }
            }
        }
    ...
    private int prepareChooseLabelLevelDialog(int node) {

        int visibleDepth = m_graph.getNodeVisibleDepth(node);

        int selectedLevel = 0;
        if (visibleDepth > 0) {
            Integer[] allowableValueSet = new Integer[visibleDepth + 1];
            for (int i = 0; i <= visibleDepth; i++) {
                allowableValueSet[i] = i;
            }

            Object retval = JOptionPane.showInputDialog(m_frame, "The selected node can have labels boiled to children upto a level of " + visibleDepth + ". Input maximum level desired.", "Node Label Level", JOptionPane.QUESTION_MESSAGE, null, allowableValueSet, allowableValueSet[0]);
            if (retval != null) {
                selectedLevel = (Integer) retval;
            }
        }
    ...
    private void handleClearAllLabels() {
        m_labelledNodes.clear();

        m_eventHandler.forceIdleState();
        m_eventHandler.refreshDisplay();
        handleRefreshLabelDisplay();
    }

    private void handleRefreshLabelDisplay() {
        for (int iterator = 0; iterator
                < m_labelledNodes.size(); iterator++) {
            H3LabelNode labelledNode = m_labelledNodes.get(iterator);


            if (labelledNode.getIsLabelled()) {

                if (m_graph.checkNodeVisible(labelledNode.getNode())) {

                    boolean success = m_eventHandler.setLabel(labelledNode);
                    labelledNode.setIsLabelled(success);
                    int index = m_labelledNodes.indexOf(labelledNode);
                    if (index == -1) {
                        m_labelledNodes.add(labelledNode);
                    } else {
                        m_labelledNodes.set(index, labelledNode);
                    }
                } else {
                    m_labelledNodes.remove(labelledNode);
                }
            } else {

                //temporarily added to limit number of nodes persisted
                m_labelledNodes.remove(labelledNode);
            }
    ...
    private void handleActivateLabelRequest(int node) {
        int selectedLevel = prepareChooseLabelLevelDialog(node);
        H3LabelNode selectedNode;
        if (selectedLevel > 0) {
            selectedNode = new H3LabelNode(node, true);
        } else {
            selectedNode = new H3LabelNode(node, false);
        }
        activateLabel(selectedNode, selectedLevel);
    }

    private void handleActivateLabelAtZeroLevelRequest(int node) {
        H3LabelNode selectedNode = new H3LabelNode(node, false);
        activateLabel(selectedNode, 0);
    }

    private void handleDeActivateLabelRequest(int node, boolean disableOnChildren) {
        H3LabelNode wrapNode = new H3LabelNode(node, false);
        int actualIndex = m_labelledNodes.indexOf(wrapNode);
        H3LabelNode actualNode = null;

        if (actualIndex != -1) {
            actualNode = m_labelledNodes.get(actualIndex);
        }

        if (actualNode != null) {
            if (disableOnChildren) {
                deActivateLabelsOnSubTree(actualNode);
            } else {
                deActivateLabel(actualNode);
            }
    ...
    private boolean setupRendering(RenderingConfiguration renderingConfiguration) {
        boolean retval = true;
        try {
            if (m_renderingConfiguration == null || !renderingConfiguration.spanningTree.equals(m_renderingConfiguration.spanningTree)) {
                m_displayPosition = null;
                m_savedDisplayPosition = null;
                m_isDisplayNarrowed = false;
                ...
                m_labelledNodes = new ArrayList<H3LabelNode>();
                ...
                if (numReachable == numNodes) {
                    if (layoutGraph(renderingConfiguration, false)) {
                        retval = true;
                        if (!renderingConfiguration.nodeAttributeColoring) {
                            colorNodes(renderingConfiguration.nodeColor);
                        } else {
                            colorNodesOnAttributes(renderingConfiguration.nodeColors);
                        }
                    ...
                    JOptionPane.showMessageDialog(null, msg, "Malformed Spanning Tree",
                            JOptionPane.ERROR_MESSAGE);
                }
            } else {
                ...
                if (!(renderingConfiguration.nodeAttributeColoring == m_renderingConfiguration.nodeAttributeColoring) || !renderingConfiguration.nodeColor.equalColoring(m_renderingConfiguration.nodeColor) || !renderingConfiguration.nodeColors.equals(m_renderingConfiguration.nodeColors)) {
    ...
    private boolean layoutGraph(RenderingConfiguration renderingConfiguration,
        ...
        if (!retval) {
            ...
            if (useExtendedPrecision) {
                ...
                JOptionPane dialog = new JOptionPane();
                int response = dialog.showConfirmDialog(null, msg, "Graph Layout Failed",
    ...
    private void startRendering(RenderingConfiguration renderingConfiguration) {
        ...
        if (!renderingConfiguration.nodeAttributeColoring) {
            includeNodeColor = determineWhetherToIncludeColor(renderingConfiguration.nodeColor);
        } else {
            includeNodeColor = true;
        }
        ...
        H3PointRenderList renderList = new H3PointRenderList(m_graph, useNodeSizes,
                includeNodes, includeNodeColor,
                includeTreeLinks, includeTreeLinkColor,
                includeNontreeLinks, includeNontreeLinkColor);

        renderList.setNearNodeAppearance(useNodeSizes
        ...
        if (renderingConfiguration.adaptiveRendering) {
            ...
            H3AdaptiveRenderLoop adaptive = new H3AdaptiveRenderLoop(m_graph, m_canvas, m_viewParameters,
        ...
        NarrowingEventHandler narrowingHandler = new NarrowingEventHandler() {
            ...
            public void enableSubtreeToParent(int node) {
                handleEnableSubtreeToParentRequest(node);
            }
            ...
            public void enableSubtreeToSpecificChildren(int node, int[] children) {
                handleEnableSubtreeToSpecificChildrenRequest(node, children);
            }
        };

        LabelingEventHandler labelingHandler = new LabelingEventHandler() {

            public void deActivateLabel(int node, boolean disableOnChildren) {

                handleDeActivateLabelRequest(node, disableOnChildren);
            }

            public void activateLabel(int node) {
                handleActivateLabelRequest(node);
            }

            public void activateLabelAtZeroLevel(int node) {
                handleActivateLabelAtZeroLevelRequest(node);
            }

            public void refreshLabelDisplay() {
                handleRefreshLabelDisplay();
            }
        ...
        DrillDownEventHandler drillDownHandler = new DrillDownEventHandler() {

            List< List<Integer>> nodePathLists = new LinkedList< List< Integer>>();

            private void createAndAddToPathSets(int node) {

                List<Integer> nodePathList = new LinkedList<Integer>();

                if (m_graph.getNodeParent(node) == -1) {
                    nodePathList.add(node);
                } else {
                    while (m_graph.getNodeParent(node) != -1) {
                        nodePathList.add(m_graph.getNodeParent(node));
                        node = m_graph.getNodeParent(node);
                    }
                }

                nodePathLists.add(nodePathList);
            }

            public void drillDownToChildGraph(int node) {
                // String childGraphPath = getChildGraphPath();
                try {

                    String childGraphPath = m_graphSetProperties.getGraphPath(m_graphSetProperties.getChildGraphName(m_loadedGraph));

                    if (!childGraphPath.isEmpty()) {

                        int loadedGraphMainAttributeOrder = m_graphSetProperties.getGraphMainAttributeOrder(m_loadedGraph);
                        int childGraphMainAttributeOrder = m_graphSetProperties.getGraphMainAttributeOrder(m_loadedGraph);
                        String childGraphName = m_graphSetProperties.getChildGraphName(m_loadedGraph);

                        String nodeFullIdentifier = m_graph.getNodeFullIdentifier(node);

                        String[] nodeFullIdentifierSplit = nodeFullIdentifier.split(";");
                        handleStopRenderingRequest();
                        handleCloseFileRequest();
                        handleOpenDiskFileRequest(childGraphPath);
                        m_loadedGraph = childGraphName;
                        handleStartRenderingRequest();

                        HashMap<Integer, String> matchingIdentifiers = m_eventHandler.searchNodesAtSpecificAttribute(nodeFullIdentifierSplit[loadedGraphMainAttributeOrder], childGraphMainAttributeOrder);

                        nodePathLists.clear();

                        int[] matchingNodeArray = new int[matchingIdentifiers.size()];
                        int count = 0;
                        for (Integer key : matchingIdentifiers.keySet()) {
                            createAndAddToPathSets(key);
                            m_eventHandler.m_labelingHandler.activateLabelAtZeroLevel(key);
                            matchingNodeArray[count++] = key;
                        }

                        HashSet<Integer> distinctNodeParentArray = new HashSet<Integer>();

                        for (List<Integer> nodePathList : nodePathLists) {
                            distinctNodeParentArray.add(nodePathList.get(0));
                        }
                        m_graph.setNodeDisplayability(false);
                        m_graph.setLinkDisplayability(false);

                        for (Integer nodeParent : distinctNodeParentArray) {
                            m_eventHandler.m_narrowingHandler.enableSubtreeToParent(nodeParent);
                            m_eventHandler.m_narrowingHandler.enableSubtreeToSpecificChildren(nodeParent, matchingNodeArray);
                        }

                        for (int matchingNode : matchingNodeArray) {
                            m_eventHandler.m_narrowingHandler.enableSubtreeToSpecificChildren(matchingNode, null);
                        }
                        nodePathLists.clear();
                    }
                } catch (Exception e) {
            ...
            public void drillDownGraphToChildren(int[] nodeArray, boolean includeChildren) {
                try {
                    if (nodeArray != null) {
                        ...
                        for (int i = 0; i < nodeArray.length; i++) {
                            createAndAddToPathSets(nodeArray[i]);
                            m_eventHandler.m_labelingHandler.activateLabelAtZeroLevel(nodeArray[i]);
                        }
                        ...
                        for (Integer nodeParent : distinctNodeParentArray) {
                            m_eventHandler.m_narrowingHandler.enableSubtreeToParent(nodeParent);
                            m_eventHandler.m_narrowingHandler.enableSubtreeToSpecificChildren(nodeParent, nodeArray);
                        }
                        if (includeChildren) {
                            for (int matchingNode : nodeArray) {
                                m_eventHandler.m_narrowingHandler.enableSubtreeToSpecificChildren(matchingNode, null);
                            }
                        }
                        nodePathLists.clear();
        ...
        m_eventHandler = new EventHandler(m_viewParameters, m_canvas, m_renderLoop,
                narrowingHandler,
                ...
                renderingConfiguration.automaticRefresh, m_frame, labelingHandler, drillDownHandler, m_graphSetProperties, m_loadedGraph);
        ...
        m_eventHandler.m_labelingHandler.activateLabelAtZeroLevel(m_rootNode);
    ...
    ///////////////////////////////////////////////////////////////////////
    // Changes the enabled state of various attributeMenu items to reflect an idle
    ...
    ///////////////////////////////////////////////////////////////////////
    // Changes the enabled state of various attributeMenu items to reflect an active
    ...
    private void setupActiveRenderingMenu() {
        ...
        m_searchMenuItem.setEnabled(true);
    ...
    private JMenuBar createInitialMenuBar() {
        ...
        // Create "Rendering" attributeMenu. ----------------------------------------
        ...
        m_nodeAttributeColoringMenuItem =
                new JCheckBoxMenuItem("Node Attribute Coloring");
        m_nodeAttributeColoringMenuItem.setMnemonic(KeyEvent.VK_R);
        m_nodeAttributeColoringMenuItem.setSelected(false);
        ...
        m_searchMenuItem = new JMenuItem("Search");
        m_searchMenuItem.setMnemonic(KeyEvent.VK_Y);
        m_searchMenuItem.setEnabled(false);
        m_searchMenuItem.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {

                Object retval = JOptionPane.showInputDialog(m_frame, "Enter search key on identfiers (Min: 3 characters)", "Search Key Input", JOptionPane.QUESTION_MESSAGE);

                if (retval != null) {
                    String query = (String) retval;

                    if (!query.equals("") && query.length() > 2) {

                        HashMap<Integer, String> matchingIdentifiers = m_eventHandler.searchNodes(query);

                        if (!matchingIdentifiers.isEmpty()) {

                            Object enableLabelDisplay = JOptionPane.showInputDialog(m_frame, "List of nodes to be selected", "Nodes", JOptionPane.INFORMATION_MESSAGE, null, matchingIdentifiers.values().toArray(), matchingIdentifiers.values().toArray()[0]);

                            if (enableLabelDisplay != null) {
                                for (Integer key : matchingIdentifiers.keySet()) {
                                    m_eventHandler.m_labelingHandler.activateLabelAtZeroLevel(key);
                                }
                            } else {
                                JOptionPane.showMessageDialog(m_frame, "Unsuccessful search? Try a different query.", "Unsuccessful search", JOptionPane.INFORMATION_MESSAGE);
                            }
                        } else {
                            JOptionPane.showMessageDialog(m_frame, "No matches found!!", "No Matches Warning", JOptionPane.INFORMATION_MESSAGE);
                        }
                    } else {
                        JOptionPane.showMessageDialog(m_frame, "Please enter query string of valid length (>4)", "Invalid Length", JOptionPane.WARNING_MESSAGE);
                    }
                }
            }
        });
        ...
        m_displayMenu.addSeparator();
        m_displayMenu.add(m_searchMenuItem);

        // Create "Load" attributeMenu. ---------------------------------------------

        m_loadMenu = new JMenu("Load");
        m_loadMenu.setMnemonic(KeyEvent.VK_L);
        createLoadMenuItems();
    ...
    private static final int ROUND_OFF_MASK = 100;
    ///////////////////////////////////////////////////////////////////////
    ...
    private String m_loadedGraph; // ...non-null when a graph is being rendered.

    public void setM_loadedGraph(String m_loadedGraph) {
        this.m_loadedGraph = m_loadedGraph;
    }
    private H3GraphSetProperties m_graphSetProperties;
    ...
    private Properties graphSetProperties = new Properties();
    private ArrayList<H3LabelNode> m_labelledNodes;
    ...
    private JCheckBoxMenuItem m_nodeAttributeColoringMenuItem;
    ...
    private JMenuItem m_searchMenuItem;
    private JMenu m_spanningTreeMenu;
    private JMenu m_loadMenu;
    ...
    private void createLoadMenuItems() {
        ...
        if (m_graphSetProperties.getGraphNames().length > 0) {

            for (String graphName : m_graphSetProperties.getGraphNames()) {

                JMenuItem menuItem = new JMenuItem(graphName);
                menuItem.setEnabled(false);
                String graphPath = m_graphSetProperties.getGraphPath(graphName);
                ...
                if (!graphPath.isEmpty()) {
                    ...
                    menuItem.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            String graphName = sourceMenuItem.getText();
                            String graphPath = m_graphSetProperties.getGraphPath(graphName);
                            handleOpenDiskFileRequest(graphPath);
                            m_loadedGraph = graphName;
                        }
                    });
                }
                m_loadMenu.add(menuItem);
            }
        }
    }

    private void handleOpenDiskFileRequest(String path) {
        ...
        System.out.println("Finished handleOpenDiskFileRequest()");

    }

    private void loadGraphSetProperties() {
        try {
            InputStream in = getClass().getResourceAsStream("graphSetProperties.properties");
            graphSetProperties.load(in);
        } //catch exception in case properties file does not exist
        catch (IOException e) {

            JOptionPane dialog = new JOptionPane();
            dialog.showMessageDialog(m_frame, "No Graph Configuration File Found", "Error",
                    JOptionPane.ERROR_MESSAGE);
    ...
    private static interface NarrowingEventHandler {
        ...
        void enableSubtreeToParent(int node);

        void enableSubtreeToSpecificChildren(int node, int[] children);
    ...
    private static interface LabelingEventHandler {

        void deActivateLabel(int node, boolean disableOnChildren);

        void activateLabel(int node);

        void activateLabelAtZeroLevel(int node);
    ...
    private static interface DrillDownEventHandler {

        void drillDownToChildGraph(int node);

        void drillDownGraphToChildren(int[] nodeArray, boolean includeChildren);
    }
...
//       only the AWT thread should access this class.
    private static class EventHandler
        ...
        public EventHandler(H3ViewParameters parameters,
                ...
                boolean onScreenLabels, boolean automaticRefresh, JFrame frame, LabelingEventHandler labelingHandler, DrillDownEventHandler drillDownHandler, H3GraphSetProperties graphSetProperties, String loadedGraph) {
            ...
            m_labelingHandler = labelingHandler;
            m_drillDownEventHandler = drillDownHandler;
            m_rootNode = rootNode;
            m_currentNode = currentNode;
            m_previousNode = previousNode;

            m_graphSetProperties = graphSetProperties;
            m_loadedGraph = loadedGraph;
                ...
                case STATE_IDLE:
                    ...
                    if (checkModifiers(modifiers, InputEvent.BUTTON1_MASK)) {
                        ...
                        if (checkModifiers(modifiers, InputEvent.SHIFT_MASK)) {
                            ...
                            int node = m_renderLoop.pickNode(x, y, m_center);
                            if (node >= 0) {
                                m_labelingHandler.activateLabelAtZeroLevel(node);
                                m_drillDownEventHandler.drillDownToChildGraph(node);
                            }
                        }
                    } //right mouse button
                    else if (checkModifiers(modifiers, InputEvent.BUTTON3_MASK)) {
                        if (checkModifiers(modifiers, InputEvent.SHIFT_MASK)) {
                            if (checkModifiers(modifiers, InputEvent.CTRL_MASK)) {

                                if (checkModifiers(modifiers, InputEvent.META_MASK)) {
                                    int node = m_renderLoop.pickNode(x, y, m_center);

                                    System.out.println("Deselect all labels");
                                    m_labelingHandler.clearAllLabels();
                                    m_labelingHandler.activateLabelAtZeroLevel(m_rootNode);
                                    ...
                                    int node = m_renderLoop.pickNode(x, y, m_center);
                                    if (node > 0) {
                                        System.out.println("Deselect label hierarchially");
                                        m_labelingHandler.deActivateLabel(node, true);
                                ...
                                m_state = STATE_DISPLAYING_LABELS;
                                int node = m_renderLoop.pickNode(x, y, m_center);
                                if (node > 0) {
                                    displayAttributes(x, y);
                                    m_labelingHandler.activateLabel(node);
                                }
                            }
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            switch (m_state) {
                ...
                case STATE_DISPLAYING_LABELS:
                    ...
                    m_labelingHandler.refreshLabelDisplay();
                    m_state = STATE_IDLE;
                ...
                //FALLTHROUGH
                default:
                    throw new RuntimeException("Invalid state in EventHandler: mouseReleased in state "
                            + m_state);
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            switch (m_state) {
                ...
                //FALLTHROUGH
                default:
                    throw new RuntimeException("Invalid state in EventHandler: mouseDragged in state "
                            + m_state);
        ...
        public void mouseMoved(MouseEvent e) {
            ...
            switch (m_state) {
                ...
                default:
                    throw new RuntimeException("Invalid state in EventHandler: mouseMoved in state "
                            + m_state);
        ...
        private void displayAttributes(int x, int y) {
            ...
            int node = m_renderLoop.pickNode(x, y, m_center);
            if (node >= 0) {
                String statusBarLabel = getAllPropertyLabel(node);
        ...
        private void displayAttributesOnStatusBar(int x, int y) {
            m_renderLoop.highlightNode(x, y);
        ...
        private void displayOnScreenLabel(int x, int y, String label) {
            ...
            m_labelZOffsetCounter++;
            m_parameters.drawLabel(gc, position.x, position.y, m_labelZOffsetCounter++, label);
        }

        private boolean setLabel(H3LabelNode labelNode) {
            boolean success = false;
            try {
                m_renderLoop.highlightNode(labelNode.getNode());

                String nodeFullIdentifier = m_graph.getNodeFullIdentifier(labelNode.getNode());
                String[] nodeFullIdentifierSplit = nodeFullIdentifier.split(";");

                if (m_onScreenLabels) {

                    String onScreenLabel = nodeFullIdentifierSplit[m_graphSetProperties.getGraphMainAttributeOrder(m_loadedGraph)];
                    success = displayOnScreenLabel(labelNode.getNode(), onScreenLabel);
                }
        ...
        public void transformPt(Point3d localPt, Point windowPt, Transform3D localToImagePlate) {

            Point3d imagePlatePt = new Point3d();
            localToImagePlate.transform(localPt, imagePlatePt);

            Point3d eyePos = new Point3d();
            m_canvas.getCenterEyeInImagePlate(eyePos);
            ...
            double physicalScreenWidth =
                    m_canvas.getScreen3D().getPhysicalScreenWidth();
            double physicalScreenHeight =
                    m_canvas.getScreen3D().getPhysicalScreenHeight();
            Dimension screenSize = m_canvas.getScreen3D().getSize();
            double metersPerPixelX = physicalScreenWidth / (double) screenSize.width;
            double metersPerPixelY = physicalScreenHeight / (double) screenSize.height;


            screenPt.x = eyePos.x + projVec.x * zScale;
            screenPt.y = eyePos.y + projVec.y * zScale;
            Point canvasScr = m_canvas.getLocationOnScreen();
        ...
        private HashMap<Integer, String> searchNodes(String query) {


            HashMap<Integer, String> matchingIdentifiers = new HashMap<Integer, String>();
            ...
            for (int i = 0; i < numNodes; i++) {
                ...
                if (m_graph.checkNodeVisible(node)) {
                    ...
                    String nodeFullIdentifier = m_graph.getNodeFullIdentifier(node);
                    if (nodeFullIdentifier.toLowerCase().contains(query.toLowerCase())) {
                        matchingIdentifiers.put(node, nodeFullIdentifier);
                    }
                }
            }
        ...
        private HashMap<Integer, String> searchNodesAtSpecificAttribute(String query, int attributeOrder) {
            ...
            for (int i = 0; i < numNodes; i++) {
                ...
                if (m_graph.checkNodeVisible(node)) {
                    ...
                    String nodeFullIdentifier = m_graph.getNodeFullIdentifier(node);
                    String[] nodeFullIdentifierSplit = nodeFullIdentifier.split(";");
                    if (attributeOrder < nodeFullIdentifierSplit.length) {
                        if (nodeFullIdentifierSplit[attributeOrder].toLowerCase().contains(query.toLowerCase())) {
                            matchingIdentifiers.put(node, nodeFullIdentifier);
                        }
                    }
                }
        ...
        private String createOnScreenLabel(int node) {
            ...
            for (int i = 0; i
                    < m_nodeLabelAttributes.length; i++) {
                if (i > 0) {
                    buffer.append("  ");
                }
        ...
        private String getAllPropertyLabel(int node) {
            int nodeID = m_graph.getNodeID(node);
            return m_graph.getNodeFullIdentifier(nodeID);
        }
        ...
        private void dumpMouseEvent(String name, MouseEvent e) {
            ...
            int modifiers = e.getModifiers();
            System.out.print(checkModifiers(modifiers, InputEvent.BUTTON1_MASK)
                    ? "1" : "-");
            System.out.print(checkModifiers(modifiers, InputEvent.BUTTON2_MASK)
                    ? "2" : "-");
            System.out.print(checkModifiers(modifiers, InputEvent.BUTTON3_MASK)
                    ? "3" : "-");

            System.out.print(checkModifiers(modifiers, InputEvent.SHIFT_MASK)
                    ? "S" : "-");
            System.out.print(checkModifiers(modifiers, InputEvent.CTRL_MASK)
                    ? "C" : "-");
        ...
        private static final int STATE_DISPLAYING_LABELS = 8;
        ...
        private H3GraphSetProperties m_graphSetProperties;
        private LabelingEventHandler m_labelingHandler;
        private DrillDownEventHandler m_drillDownEventHandler;
        private int m_rootNode;
        private int m_currentNode;
        private int m_previousNode;
...
///////////////////////////////////////////////////////////////////////
    private static class NodeLabelConstructor {
        ...
        private void addAttributeValue(StringBuffer buffer, ValueIterator iterator,
...
///////////////////////////////////////////////////////////////////////
    private static class ColorSchemeMenu {
        ...
        public ColorSchemeMenu() {
            ...
            m_coloringNodeMenu = new JMenu("Node Attribute Color");




            m_coloringNodeMenu.setMnemonic(KeyEvent.VK_R);
            m_coloringNodeMenu.setEnabled(false);

            m_coloringNodeAttributeMenu = new JMenu("Coloring Node Attribute");
            m_coloringNodeAttributeMenu.setMnemonic(KeyEvent.VK_G);
            m_coloringNodeAttributeMenu.setEnabled(false);



            m_colorSchemeMenu.add(m_coloringNodeAttributeMenu);
            m_colorSchemeMenu.add(m_coloringNodeMenu);
        ...
        public JMenu getColoringNodeAttributeMenu() {
            return m_coloringNodeAttributeMenu;
        }

        public JMenu getColoringNodeMenu() {
            return m_coloringNodeMenu;
        }
        ...
        public void populateAttributeMenus(H3GraphLoader graphLoader, Graph graph) {
            ...
            populateColoringNodeAttributeMenus();

        }

        private String[] getDistinctAttributeValues(String name) {
            return m_attributeCache.getDistinctAttributeValues(name);
        }

        private void populateColoringNodeAttributeMenus() {

            List coloringAttributes = m_attributeCache.getColoringAttributes();
            nodeAttributeNames = new ArrayList<String>();
            //nodeAttributeMaps = new ArrayList<HashMap<String, ColorSelection>>();

            if (coloringAttributes.size() > 0) {
                m_coloringNodeAttributeMenu.setEnabled(true);
            } else {
                m_coloringNodeAttributeMenu.setEnabled(false);
            }
            ListIterator iterator = coloringAttributes.listIterator();
            m_coloringNodeAttributeButtonGroup = new ButtonGroup();
            while (iterator.hasNext()) {
                String name = (String) iterator.next();
                nodeAttributeNames.add(name);
                JRadioButtonMenuItem attributeMenu = new JRadioButtonMenuItem(name);

                attributeMenu.addActionListener(new ActionListener() {

                    public void actionPerformed(ActionEvent e) {
                        JRadioButtonMenuItem attributeMenu = (JRadioButtonMenuItem) e.getSource();
                        if (attributeMenu.isSelected()) {
                            String name = attributeMenu.getText();

                            selectedNodeAttribute = name;

                            String[] nodeAttributes = getDistinctAttributeValues(name);
                            m_coloringNodeMenu.removeAll();
                            m_coloringNodeMenu.setEnabled(false);

                            nodeAttributeMap = new HashMap<String, ColorSelection>();
                            if (nodeAttributes != null) {
                                ...
                                for (int i = 0; i < nodeAttributes.length; i++) {
                                    ...
                                    Map nodeAttributeMenuMap = new HashMap();
                                    nodeAttributeMap.put(nodeAttributes[i], new ColorSelection(attributeValueMenu, nodeAttributeMenuMap, m_fixedColors, true));

                                    m_coloringNodeMenu.add(attributeValueMenu);

                                }
                                m_coloringNodeMenu.setEnabled(true);

                            }


                        } else if (!attributeMenu.isSelected()) {
                            m_coloringNodeMenu.removeAll();
                            m_coloringNodeMenu.setEnabled(false);
                ...
                m_coloringNodeAttributeButtonGroup.add(attributeMenu);
                m_coloringNodeAttributeMenu.add(attributeMenu);

            }
        }

        public HashMap<Integer, ColorConfiguration> createNodeColorsConfigurationSnapshot(H3GraphLoader graphLoader,
                Graph graph) {

            m_attributeCache = new AttributeCache(graphLoader, graph);

            HashMap<Integer, ColorConfiguration> nodeColorsConfiguration = new HashMap<Integer, ColorConfiguration>();
            if (selectedNodeAttribute != null) {
                for (int i = 0; i < graph.getNumNodes(); i++) {
                    int node = graph.getNode(i).getObjectID();

                    String nodeAttributeValue = m_attributeCache.getDistinctAttributeValue(node, selectedNodeAttribute);
                    ColorConfiguration nodeColorConfiguration = nodeAttributeMap.get(nodeAttributeValue).createColorConfigurationSnapshot();

                    nodeColorsConfiguration.put(node, nodeColorConfiguration);

                }
            }

            return nodeColorsConfiguration;
        }
        ...
        public void removeAttributeMenus() {
            ...
            m_coloringNodeAttributeMenu.removeAll();
            m_coloringNodeAttributeMenu.setEnabled(false);
            m_coloringNodeAttributeButtonGroup = null;
        ...
        public void enableDefaultColorScheme() {
            if (m_predefinedColorSchemes.size() > 0) {
                // XXX: We might want to preserve the default colorNumeric scheme
        ...
        // PRIVATE METHODS
        ////////////////////////////////////////////////////////////////////
        private void createPredefinedColorSchemes(Map nodeMenuMap, Map treeLinkMenuMap, Map nontreeLinkMenuMap) {
            ...
            m_predefinedColorSchemes.add(maker.make("Yellow-Green-[Grey]",
            ...
            m_predefinedColorSchemes.add(maker.make("Green-Olive-[Summer Sky]",
                    "Green", ColorSchemeMaker.DISABLE_TRANSPARENCY,
                    "Olive", ColorSchemeMaker.DISABLE_TRANSPARENCY,
                    "Summer Sky",
                    ColorSchemeMaker.ENABLE_TRANSPARENCY));
            m_predefinedColorSchemes.add(maker.make("Invisible-Olive-[Summer Sky]",
                    ColorSelection.INVISIBLE,
                    ColorSchemeMaker.IGNORE_TRANSPARENCY,
                    "Olive", ColorSchemeMaker.DISABLE_TRANSPARENCY,
                    "Summer Sky",
                    ColorSchemeMaker.ENABLE_TRANSPARENCY));
            m_predefinedColorSchemes.add(maker.make("Beige-Gold-[Green]",
        ...
        private JMenu m_coloringNodeMenu;
        private JMenu m_coloringNodeAttributeMenu;
        private ButtonGroup m_coloringNodeAttributeButtonGroup;
        ...
        private HashMap<String, ColorSelection> nodeAttributeMap = new HashMap<String, ColorSelection>();
        ArrayList<String> nodeAttributeNames;
        String selectedNodeAttribute;

        // ArrayList<HashMap<String, ColorSelection>> nodeAttributeMaps;
        ////////////////////////////////////////////////////////////////////
        ...
        private class ColorSchemeMaker {
            ...
            public ColorSchemeMaker(Map nodeMenuMap, Map treeLinkMenuMap, Map nontreeLinkMenuMap) {
            ...
            public PredefinedColorScheme make(String name, String nodeColor, int nodeTransparencyType,
                ...
                return new PredefinedColorScheme(name, nodeMenuItem, nodeTransparencyMenuItem,
...
///////////////////////////////////////////////////////////////////////
    private static class ColorSelection {
        // Compile out the HUE colorNumeric choice for now.
        ...
        public ColorSelection(JMenu menu, Map map, List fixedColors) {
            this(menu, map, fixedColors, false);
        }

        public ColorSelection(JMenu menu, Map map, List fixedColors, boolean limitToInvisibleAndFixedColor) {
            // NOTE: In the following, we arbitrarily choose the invisible
            //       colorNumeric to be the default selection.  As a result, all
            //       attributeMenu items excluded by this choice are manually disabled
            ...
            m_NumericMenuItem = new JRadioButtonMenuItem(NUMERIC);
            m_NumericMenuItem.setMnemonic(KeyEvent.VK_I);
            m_NumericMenuItem.setEnabled(false);
            //.setEnabled(true);
            m_NumericMenuItem.addActionListener(new ActionListener() {

                public void actionPerformed(ActionEvent e) {
                    handleNumericColorRequest();
                }
            });
            ...
            m_colorSchemeButtonGroup.add(m_NumericMenuItem);
            ...
            putChecked(map, NUMERIC, m_NumericMenuItem);
            if (!limitToInvisibleAndFixedColor) {
        ...
        public void populateAttributeMenus(AttributeCache attributeCache) {
            ...
            m_NumericColorAttributeButtonGroup = new ButtonGroup();
            m_NumericColorAttributeMenus = createColorAttributeMenuCache(attributeCache.getNumericAttributes(),
                    m_NumericColorAttributeButtonGroup);
        ...
        // Set the colorNumeric selection to one that is independent of the content
        // of a graph.  One of the fixed colors or the invisible choice will
        // be selected.
        //
        // This step is necessary after unloading a graph so that the colorNumeric
        // selection doesn't refer to nonexistent attribute data.
        public void enableReasonableSelection() {
            if (m_hueMenuItem.isSelected() || m_RGBMenuItem.isSelected() || m_NumericMenuItem.isSelected()) {
                m_defaultSelection.setSelected(true);
        ...
        public ColorConfiguration createColorConfigurationSnapshot() {
            ...
            if (m_invisibleMenuItem.isSelected()) {
                ...
                retval.colorAttribute =
                        findSelectedMenuItem(m_colorAttributeMenu).getText();
            } else if (m_NumericMenuItem.isSelected()) {
                retval.scheme = ColorConfiguration.NUMERIC;
                retval.numericAttribute =
                        findSelectedMenuItem(m_colorAttributeMenu).getText();
            } else {
            ...
            if (m_selectionAttributeMenu.isEnabled()) {
                ...
                // NOTE: One attributeMenu item is always selected.
                // The zeroth element in the attributeMenu stands for no-choice.
        ...
        private void handleNumericColorRequest() {
            installNumericAttributeMenu();
            setupForArbitraryColorChoice();
        }
        ...
        private void removeColorAttributeMenu() {
            ...
            m_NumericColorAttributeButtonGroup = null;
            m_scalarColorAttributeMenus = null;
            m_RGBColorAttributeMenus = null;
            m_NumericColorAttributeMenus = null;
        }
        ...
        // The colorNumeric attributeMenu items aren't independent of each other.  Thus,
        ...
        // or by selecting or de-selecting, any affected attributeMenu items.
        //
        // The most common initiating change is the addition or removal
        // of the various menus listing the available colorNumeric attributes
        ...
        // to be addressed.  The first concerns the colorNumeric attribute menus,
        ...
        // the selection attribute attributeMenu.  This attributeMenu should be enabled if,
        // and only if, there are suitable attributes, and the current
        // colorNumeric choice is not "invisible."  The final consistency concerns
        ...
        private void updateMenuInterdependencies() {
            ...
            m_NumericMenuItem.setEnabled(m_NumericColorAttributeMenus != null);
            if (m_NumericMenuItem.isEnabled() && m_NumericMenuItem.isSelected()) {
                enableColorAttributes = true;
                installNumericAttributeMenu();
            }
            ...
            if ((m_hueMenuItem.isSelected() && !m_hueMenuItem.isEnabled())
                    || (m_RGBMenuItem.isSelected() && !m_NumericMenuItem.isEnabled())) {
                m_defaultSelection.setSelected(true);
            }
        ...
        private void installNumericAttributeMenu() {
            m_colorAttributeMenu.removeAll();
            for (int i = 0; i < m_NumericColorAttributeMenus.length; i++) {
                m_colorAttributeMenu.add(m_NumericColorAttributeMenus[i]);
            }
        }

        /**
         * Update <code>m_selectedFixedColorIndex</code> if the given
         * selected attributeMenu item is that for a fixed colorNumeric.
         *
         * <p>
         * Usually, this value is automatically updated by an action
         * listener added to the attributeMenu item representing a fixed
         * colorNumeric.  That works well in general, but during program
         * initialization, when we want to setup the default colorNumeric scheme,
        ...
        private JMenuItem findSelectedMenuItem(JMenu menu) {
            ...
            for (int i = 0; i < numItems && retval == null; i++) {
                JMenuItem menuItem = menu.getItem(i);
                // NOTE: getItem() returns null for attributeMenu separators.
                if (menuItem != null && menuItem.isSelected()) {
                    retval = menuItem;
                }
            }
        ...
        private JRadioButtonMenuItem m_NumericMenuItem;
        ...
        // This is the attributeMenu item which will be in the selected state when
        // no attributeMenu item has yet been selected by the user or when the
        // currently selected attributeMenu item must be disabled (as a result of
        // removing all colorNumeric attributes, for example).  This must point
        // to a attributeMenu item for a fixed colorNumeric or the invisible choice.
        // By doing so, we ensure that this attributeMenu item is selectable in
        ...
        // the current attributeMenu selection, either m_scalarColorAttributeMenus or
        ...
        // latter set of attributeMenu items will be so installed.  If m_hueMenuItem
        ...
        private ButtonGroup m_NumericColorAttributeButtonGroup;
        private JRadioButtonMenuItem[] m_scalarColorAttributeMenus;
        private JRadioButtonMenuItem[] m_RGBColorAttributeMenus;
        private JRadioButtonMenuItem[] m_NumericColorAttributeMenus;
...
///////////////////////////////////////////////////////////////////////
    private static class RenderingConfiguration {
        ...
        public boolean nodeAttributeColoring;
        ...
        public HashMap<Integer, ColorConfiguration> nodeColors;
        ...
        public void print() {
            ...
            System.out.println("Coloring on node attributes");
            System.out.println("\tnodeAttributeColoring:" + nodeAttributeColoring);
            if (nodeAttributeColoring) {
                System.out.println("\tNode Colors:");
                /* for (Map.Entry<Integer, ColorConfiguration> entry : nodeColors.entrySet()) {
    ...
    private static class ColorConfiguration {
        ...
        public String numericAttribute;
        ...
        public boolean equalColoring(ColorConfiguration rhs) {
            ...
            if (retval) {
                if (scheme == FIXED_COLOR) {
                    ...
                    retval = (rhs.numericAttribute.equals(numericAttribute));
                }
            }
        ...
        public int hashCode() {
            int hash = 3;
            hash = 41 * hash + this.scheme;
            hash = 41 * hash + this.fixedColor;
            hash = 41 * hash + (this.isTransparent ? 1 : 0);
            hash = 41 * hash + (this.colorAttribute != null ? this.colorAttribute.hashCode() : 0);
            hash = 41 * hash + (this.selectionAttribute != null ? this.selectionAttribute.hashCode() : 0);
            hash = 41 * hash + (this.numericAttribute != null ? this.numericAttribute.hashCode() : 0);
            return hash;
        }
        ...
        public boolean equals(Object aThat) {
            ...
            if (!(aThat instanceof ColorConfiguration)) {
                return false;
            }
            ColorConfiguration that = (ColorConfiguration) aThat;

            return that.equalColoring(this);
        ...
        private static final String[] m_schemeNames = {
            "INVISIBLE", "FIXED_COLOR", "HUE", "RGB", "NUMERIC"
        };
    }
...
///////////////////////////////////////////////////////////////////////
    private static class AttributeCache {
        ...
        public AttributeCache(H3GraphLoader graphLoader, Graph graph) {
            ...
            m_coloringAttributes = graphLoader.loadColoringTreeQualifiers(graph);
            m_numericAttributes = graphLoader.loadNumericTreeQualifiers(graph);
        }
        ...
        public List getColoringAttributes() {
            return m_coloringAttributes;
        }
        ...
        private String getDistinctAttributeValue(int node, String attributeName) {
            AttributeDefinitionIterator adi = graph.getAttributeDefinition(attributeName);
            NodeLabelConstructor nodeAttributeInterface = new NodeLabelConstructor(graph, new int[]{adi.getID()});
            int nodeId = graph.getNode(node).getObjectID();
            return nodeAttributeInterface.extractValues(nodeId, false)[0];

        }

        private String[] getDistinctAttributeValues(String attributeName) {
            AttributeDefinitionIterator adi = graph.getAttributeDefinition(attributeName);
            NodeLabelConstructor nodeAttributeInterface = new NodeLabelConstructor(graph, new int[]{adi.getID()});
        ...
        private List m_coloringAttributes;
        private List m_numericAttributes;
        private H3GraphLoader.AttributeTypeMatcher m_scalarColorAttributeTypeMatcher =
                new ScalarColorAttributeTypeMatcher();
        private H3GraphLoader.AttributeTypeMatcher m_RGBColorAttributeTypeMatcher =
                new RGBColorAttributeTypeMatcher();
        private H3GraphLoader.AttributeTypeMatcher m_selectionAttributeTypeMatcher =

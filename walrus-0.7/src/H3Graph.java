...
public class H3Graph {
    ...
    //This method returns maximum number of levels of visible children
    public int getNodeVisibleDepth(int node) {
        return m_nodes.visibleDepth[node];
    }
    //This method returns the colon seperated concatenated value of all attributes of the node as node identifier

    public String getNodeFullIdentifier(int node) {
        return m_nodes.fullIdentifier[node];
    }
    //This method returns the node identifiers for all nodes

    public String[] getNodeFullIdentifiers() {
        return m_nodes.fullIdentifier;
    }
    //------------------
    ...
    //This method sets the visible depth for a node
    public void setNodeVisibleDepth(int node, int visibleDepth) {
        m_nodes.visibleDepth[node] = visibleDepth;

    }
    //This method sets the full identfier( colon seperated concatenated values of all attibutes of a node) of a node

    public void setNodeFullIdentifier(int node, String fullIdentifier) {
        m_nodes.fullIdentifier[node] = fullIdentifier;
    }
    ...
    //This method defaults the visible depth of all nodes to 0

    public void initNodeVisibleDepth() {
        Arrays.fill(m_nodes.visibleDepth, 0);
    }
    ...
    public void enableVisibilityToParent(int node) {
    ...
    public void enableVisibilityToSpecificChildren(int node, int[] children) {

        if (children != null && children.length > 0) {
            Arrays.sort(children);
        }
        setSelectedSubtreeDisplayability(node, true, children);
        computeVisibility();
        computeVisibleDepth();
    ...
    public void computeVisibleDepth() {

        computeNodeVisibleDepth(m_rootNode);
    }
    ...
    private int computeNodeVisibleDepth(int node) {
        int start = getNodeChildIndex(node);
        int nontreeStart = getNodeNontreeIndex(node);
        boolean childAlive = false;

        if (nontreeStart - start <= 0) {
            return 0;
        }

        int maxVisibleChildDepth = 0;

        for (int i = start; i < nontreeStart; i++) {
            int child = getLinkDestination(i);
            if (checkNodeVisible(child)) {
                childAlive = true;
                int childVisibleDepth = computeNodeVisibleDepth(child);
                if (maxVisibleChildDepth < childVisibleDepth) {
                    maxVisibleChildDepth = childVisibleDepth;
                }
            }
        }
        if (childAlive) {
            setNodeVisibleDepth(node, maxVisibleChildDepth + 1);
            return maxVisibleChildDepth + 1;
        } else {
            setNodeVisibleDepth(node, 0);
            return 0;
        }
    ...
    private void setSelectedSubtreeDisplayability(int node, boolean isDisplayable, int[] sortedNodeArray) {
        int start = getNodeChildIndex(node);
        int end = getNodeLinksEndIndex(node);
        int nontreeStart = getNodeNontreeIndex(node);
        ...
        for (int i = start; i < nontreeStart; i++) {
            ...
            if (sortedNodeArray != null && sortedNodeArray.length > 0) {

                if (Arrays.binarySearch(sortedNodeArray, child) >= 0) {

                    setNodeDisplayability(child, isDisplayable);

                    setLinkDisplayability(i, isDisplayable);

                    setSelectedSubtreeDisplayability(child, isDisplayable, sortedNodeArray);
                }
            } else {
                setNodeDisplayability(child, isDisplayable);
    ...
    private static class Nodes {
        ...
        public Nodes(int numNodes) {
            ...
            visibleDepth = new int[numNodes];
            fullIdentifier = new String[numNodes];
        ...
        public int[] linksEnd;
        //the visible depth property of a node is reference to maximum length of the visible paths
        //from node to a non tree link (leaf)
